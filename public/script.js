...
<div>
    <label class="sr-only" for="search">Search</label>
        <div :class="headerClasses" class="flex items-center relative sticky top-0 pt-3 transition-opacity duration-300 rounded-full" id="header">
            <input v-model="search" ref="searchInput" :disabled="loading" :placeholder="loading ? 'loading data…' : 'type a search query…'" class="border border-gray-600 placeholder-gray-600 w-full rounded-full py-2 px-6 outline-none focus:border-blue-300" type="text" id="search"/>
            <button v-show="search" @click="reset(true)" class="absolute right-0 rounded-full text-gray-600 focus:outline-none hover:text-red-800 hover:bg-red-200 p-2 mr-1" title="Clear (Esc)">
