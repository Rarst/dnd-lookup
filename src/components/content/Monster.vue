<template>
  <div class="mt-3 border rounded-lg py-3 px-6 space-y-3">
    ...
    <p>
      <strong class="font-semibold">Hit Points</strong> {{ item.hit_points }} ({{ item.hit_dice }}<span v-if="hitBonus"> {{ hitBonus > 0 ? '+' : '-' }} {{ Math.abs(hitBonus) }}</span>)
    </p>
    <p>
    ...
    <p v-if="item.damage_vulnerabilities.length">
      <strong class="font-semibold">Damage vulnerabilities</strong> {{ item.damage_vulnerabilities.join(', ') }}</p>
    <p v-if="item.damage_resistances.length">
      <strong class="font-semibold">Damage resistances</strong> {{ item.damage_resistances.join(', ') }}</p>
    <p v-if="item.damage_immunities.length">
      <strong class="font-semibold">Damage immunities</strong> {{ item.damage_immunities.join(', ') }}</p>
    <p v-if="item.condition_immunities.length">
      <strong class="font-semibold">Condition immunities</strong> {{ item.condition_immunities.map(ci => ci.name).join(', ') }}
    </p>
    <p>
    ...
    <p v-for="action in item.actions">
      <strong class="font-semibold">{{ action.name }}<span v-if="action.usage"> ({{ action.usage.times }} {{ action.usage.type }})</span>.</strong> {{ action.desc }}
    </p>

    <template v-if="item.legendary_actions">
      <h3 class="font-semibold text-lg">Legendary Actions</h3>
      <p v-for="action in item.legendary_actions">
        <strong class="font-semibold">{{ action.name }}.</strong> {{ action.desc }}</p>
    </template>
  </div>
</template>
