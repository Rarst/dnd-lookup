<template>
  ...
  <p>
    <strong class="font-semibold">Hit Points</strong> {{ item.hit_points }} ({{
      item.hit_dice
    }}<span v-if="hitBonus">
  ...
  <div class="flex justify-between text-center">
    ...
    >
    <span
      ><strong class="font-semibold">DEX</strong><br />{{ item.dexterity }} ({{
  ...
  <p>
    <strong class="font-semibold">Challenge</strong> {{ item.challenge_rating }}
  </p>
  ...
  <template v-if="item.legendary_actions">
    <h3 class="text-lg font-semibold">Legendary Actions</h3>
